...
TRAIN_EPISODES = 15     # total number of episodes in each season
...
def read():
    ...
    decoded_values = str(value[0:len(value)].decode("utf-8"))
...
def writes(x):
    ...
    arduino.write(bytes(str((x+255)*10), 'utf-8'))
    #start_time = time.time()
    

def get_reward(th,thdot, u):
    #print(th, thdot, u)
    #u = np.clip(u, -255, 255)[0]
    costs = (10 * th ** 2 + 0.01 * thdot ** 2 + 0.00001 * (u ** 2))
    #print(costs)
    return -costs
...
def get_state(angle, angular_velocity):
    offset = -7.5 * np.pi/180
    angle = angle + offset
    sin_theta = np.sin(angle)
    cos_theta = np.cos(angle)
    return [cos_theta, sin_theta, angular_velocity]

def step_forward(action):
    # communication with arduino
    writes(int(action))
    angle, angular_velocity, action = read()
    # print(" Arduino received :     ", action)
    reward = get_reward(angle*np.pi/180, angular_velocity * np.pi/180, action)
    return angle*np.pi/180, angular_velocity*np.pi/180, reward
...
#     print(angular_velocity)
    return angle*np.pi/180, angular_velocity*np.pi/180

# def collect_trajectories(env, agent, max_episodes):
def collect_trajectories(agent, max_episodes):
  ep_reward_list = []
  steps = 0
  done = False  
  for ep in range(max_episodes):
    angle, angular_velocity = get_initial_state()
    state = get_state(angle, angular_velocity)
    t = 0
    ep_reward = 0
    while True:
      action = agent.policy(state)
      action = int(action)
      angle, angular_velocity, reward = step_forward(action)
      next_state = get_state(angle, angular_velocity)
      if check_angle(angle):done = True  
...
def test(agent):  
    ...
    print('---------------------------')
    path = './'
    agent.load_model(path, 'actor_weights (1).h5', 'critic_weights (1).h5')
    ep_reward_list = []
    action = None
    for ep in range(10):
        #print(ep)
        ...
        while True:
            action = agent.policy(state)
            action = int(action * 127.5)
            action = np.clip(action, -255, 255)
            #print(time.time() - start_time)
            #while (time.time() - start_time)<0.036:
            #time.sleep(0.01) 
            #print('Angle {} ,  Angular_velocity {} , Action {}'.format(angle*180/np.pi,angular_velocity*180/np.pi, action))
            angle, angular_velocity, reward = step_forward(action)
            print(time.time() - start_time,',',angle*180/np.pi,',',angular_velocity*180/np.pi,',', action,',',reward)
            start_time = time.time()
            #print(reward)
            ...
            #print(t)
            state = next_state
            if t==20000 or check_angle(angle):
                ep_reward_list.append(ep_reward)
...
if __name__ == '__main__':
    ...
    print(action_bound)
    # create an agent
    arduino = serial.Serial('COM10',2000000)
    ...
    #agent.load_model('./', 'actor_weights (1).h5', 'critic_weights (1).h5')
